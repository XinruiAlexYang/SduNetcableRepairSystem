...
function getHead(result){
    ...
    result.forEach(function(item){
        ...
        if(item.indexOf('前天') != -1||item.indexOf('昨天') != -1||item.indexOf('报修日期') != -1|| (item.indexOf('月')!= -1 && (item.indexOf('日')!=-1 || item.indexOf('号')!= -1))){list.push('reportTime');console.log(item);}
        if(item.indexOf('联系方式') != -1)list.push('conPhone');
        if(item.indexOf('实际原因') != -1)list.push('repFact');
        if(item.indexOf('报修单') != -1 || item.indexOf('报修情况') != -1||item.indexOf('维修情况') != -1)list.push('all');
...
function onRequest(req, res){
    ...
    req.on('end', function(){
        ...
        var reqObj = jsonData;
        var result = segment.doSegment(reqObj,{simple:true});
